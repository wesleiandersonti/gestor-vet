    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const elements = {
                ...
                currentVersion: '{{ config("app.version") }}'.trim() // Pega a versão do .env
            };
            ...
            window.filterTransactions = async function(period) {
                try {
                    const response = await fetch(`/api/transactions?period=${period}&user_id={{ auth()->user()->id }}`);
...
@if($user->role_id == 2 && $daysUntilExpiration <= 7)
        ...
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center">
                @if($daysUntilExpiration > 0)
                    <span>
                        Seu acesso expira em <strong>{{ $daysUntilExpiration }} {{ Str::plural('dia', $daysUntilExpiration) }}</strong>.
                        Renove agora mesmo para manter seu painel ativo.
                    </span>
                @else
                    <span>
                        Seu acesso expirou em <strong class="text-danger">{{ $expirationDate->format('d/m/Y') }}</strong>.
      ...
      <div class="d-flex align-items-start gap-3">
        ...
        <div>
          <h4 class="welcome-title">
            {{ $saud }}, <span class="welcome-name">{{ $nome }}</span>!
          </h4>
          ...
          <p class="welcome-sub mb-0">{{ $mensagemDia }}</p>
        </div>
      </div>

      @if($mostrarChip)
      <div class="text-end">
        <span class="welcome-chip compact {{ $chipClasse }}">
          {!! $chipTexto !!}
        </span>
        @if(!empty($chipSubnote))
          <small class="welcome-subnote">{{ $chipSubnote }}</small>
        @endif
      </div>
      @endif
    </div>
...
{{-- ====== FIM DA MENSAGEM DE BOAS-VINDAS (PREMIUM) ====== --}}

{{-- ===== SUPORTE / COMUNIDADE (Compacto e elegante) ===== --}}
@php
  $user        = auth()->user();
  ...
  <div class="card mb-4 border-0 shadow-sm"
    ...
    <div class="card-body py-3 px-4 d-flex flex-wrap align-items-center justify-content-between gap-3">
      ...
      <div class="d-flex flex-wrap gap-2">
        <a href="{{ $waLink }}" target="_blank" class="btn btn-sm text-white" 
        ...
        <a href="{{ $canalLink }}" target="_blank" class="btn btn-sm text-white"
        ...
        <a href="{{ $grupoLink }}" target="_blank" class="btn btn-sm text-white"
      ...
      <div class="row gy-3">
        <div class="col-md-4 col-12">
          <div class="d-flex align-items-center">
            ...
            <div class="card-info">
              <h5 class="mb-0 valor-financeiro" data-realvalor="R$ {{ number_format($totalMes ?? 0, 2, ',', '.') }}">   R$ {{ number_format($totalMes ?? 0, 2, ',', '.') }} </h5>
        ...
        <div class="col-md-4 col-12">
          <div class="d-flex align-items-center">
            ...
            <div class="card-info">
              <h5   class="mb-0 valor-financeiro"   data-realvalor="R$ {{ number_format($totalAReceberMes ?? 0, 2, ',', '.') }}" >   R$ {{ number_format($totalAReceberMes ?? 0, 2, ',', '.') }} </h5>
        ...
        <div class="col-md-4 col-12">
          <div class="d-flex align-items-center">
            ...
            <div class="card-info">
              <h5   class="mb-0 valor-financeiro"   data-realvalor="R$ {{ number_format($totalHistorico ?? 0, 2, ',', '.') }}" >   R$ {{ number_format($totalHistorico ?? 0, 2, ',', '.') }} </h5>
