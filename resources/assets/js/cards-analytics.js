...
(function () {
  ...
  // Estatísticas Detalhadas - Gráfico de Barras
  // --------------------------------------------------------------------
  const detailedStatisticsChartEl = document.querySelector('#totalRevenueChart'),
    detailedStatisticsChartOptions = {
      ...
      colors: [config.colors.primary, config.colors.info, config.colors.danger, config.colors.success],
      dataLabels: {
        enabled: false
      },
      stroke: {
      ...
      yaxis: {
        ...
        min: 0,
        max: Math.max(estatisticas.totalClientes, estatisticas.inadimplentes, estatisticas.ativos, estatisticas.expiramHoje) + 50,
        tickAmount: 5
      },
      responsive: [
        {
  ...
  if (typeof detailedStatisticsChartEl !== undefined && detailedStatisticsChartEl !== null) {
    const detailedStatisticsChart = new ApexCharts(detailedStatisticsChartEl, detailedStatisticsChartOptions);
